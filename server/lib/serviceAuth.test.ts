...
describe("signServiceRequest / verifyServiceRequest", () => {
  it("round-trips a signed request", () => {
    ...
    const headers = signServiceRequest(body, SECRET, { timestamp: NOW, nonce: "abc" });
    expect(headers[TIMESTAMP_HEADER]).toBe(String(NOW));
    expect(headers[NONCE_HEADER]).toBe("abc");
  ...
  it("rejects a tampered body", () => {
    const headers = signServiceRequest("original", SECRET, { timestamp: NOW, nonce: "n1" });
  ...
  it("rejects a wrong secret", () => {
    const headers = signServiceRequest("body", SECRET, { timestamp: NOW, nonce: "n1" });
  ...
  it("rejects a replayed nonce when a guard is supplied", () => {
    const headers = signServiceRequest("body", SECRET, { timestamp: NOW, nonce: "used" });
  ...
  it("accepts case-insensitive header keys", () => {
    const body = "body";
    const signed = signServiceRequest(body, SECRET, { timestamp: NOW, nonce: "n1" });
    const upper = {
